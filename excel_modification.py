...
class FileUpdate:
    ...
    def update_facilities(self, base_dir: str, sub_directories: list):
        for i, sub in enumerate(sub_directories, start=1):
            self.sheet_cols = self.facility_cols[i]
            self.facility_cell = self.facility_name_cells[i]
            path = f'{base_dir}/{sub}/*.csv'
            glob_files = glob.glob(path)
            dataframes = []
            for file in glob_files:
                dframe = pd.read_csv(file, usecols=self.cols).fillna('')
                dataframes.append(dframe)
            temp_df = pd.concat(dataframes).reset_index(drop=True)
    ...
    def update_facility(self, df, facility_name, single=False):
        ...
        ar6[self.facility_cell] = re.sub('\d - ', '', facility_name)
